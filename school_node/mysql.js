...
export class _mysql {
    ...
    selectAllCourse(callback) {
        this.connection.query('select c.*,u.*,s.sort_name from s_course c,s_user u,s_course_sort s where c.teacher_id = u.id,and c.sort_id = s.id',
    ...
    selectAllCourseBySort(callback) {
        this.connection.query('select c.*,u.* from s_course c,s_user u,s_course_sort s where c.teacher_id = u.id and c.sort_id = ?',
    ...
    selectAllCourseById(callback) {
        this.connection.query('select c.*,u.*,s.sort_name from s_course c,s_user u,s_course_sort s where c.teacher_id = u.id and c.sort_id = s.id and c.id = ?',
    ...
    selectAllCourseByTea(callback) {
        this.connection.query('select c.*,u.*,s.sort_name from s_course c,s_user u,s_course_sort s where c.teacher_id = ? and c.sort_id = s.id and c.id = ? and c.teacher_id = u.id',
    ...
    selectAllStudentByCource(callback) {
        this.connection.query('select c.*,u.* from s_students_course_map c,s_user u where c.student_id = u.id and c.course_id = ?',
            [this.getData.courseId], function (err, rs, fields) {
                console.log(rs)
                return callback(err, rs, fields)
            })
    ...
    updateCourse(callback) {
        this.connection.query('update s_course set course_name = ?,course_info = ?,course_url = ?,course_source_url = ?,course_source_name = ?',
            [this.getData.courseName, this.getData.courseInfo, this.getData.courseUrl, this.getData.courseSourceUrl, this.getData.courseSourceName], function (err, rs, fields) {
                console.log(rs)
                return callback(err, rs, fields)
            })
    }
    ...
    addQuestion(callback) {
        this.connection.query('insert into s_question (question_article,question_type,question_answer,is_question_answer,question_erroe,course_id,score) value (?,?,?,?,?,?,?)',
    ...
    deleteQuestion(callback) {
        this.connection.query('delets from s_question where id = ?', [this.getData.questionId], function (err, rs, fields) {
            console.log(rs)
            return callback(err, rs, fields)
        })
    }
    ...
    updateQuestion(callback) {
        this.connection.query('update s_question set question_article = ?,question_trpe = ?,question_answer = ?,is_question_answer = ?,question_error = ?,score = ?',
            [this.getData.questionArticle, this.getData.questionType, this.getData.questionAnswer, this.getData.isQuestionAnswer, this.getData.questionError, this.getData.score],
            function (err, rs, fields) {
                console.log(rs)
                return callback(err, rs, fields)
            })
    ...
    selectAllSort(callback) {
        this.connection.query('select s.*,u.* from s_course_sort s,s_user u where s.user_id = u.id', function (err, rs, fields) {
            console.log(rs)
            return callback(err, rs, fields)
        })
    }
